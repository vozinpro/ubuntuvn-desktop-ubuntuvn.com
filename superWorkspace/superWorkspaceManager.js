...
const Ubuntuvn = imports.misc.extensionUtils.getCurrentExtension();
...
var SuperWorkspaceManager = class SuperWorkspaceManager {
    constructor(appsByCategory) {
        ...
        this.workspaceCategories = global.UbuntuvnDesktop.init_workspace_categories();
        this.categoryKeyOrderedList =
            Me.stateManager.getState('categoryKeyOrderedList') || [];
        this.noUImode = Ubuntuvn.noUimode;
        for (let [key, category] of Object.entries(this.workspaceCategories)) {
    ...
    updateWorkspaces(newappsByCategory) {

        this.appsByCategory = newappsByCategory;
        this.workspaceCategories = global.UbuntuvnDesktop.init_workspace_categories();
        this.prepareWorkspaces();
        let activeSuperWorkspace = this.getActiveSuperWorkspace();
        activeSuperWorkspace.uiVisible = true;
    ...
    prepareWorkspaces() {
        ...
        for (var i = 0; i < diff; i++) {
            if (
                ...
                let superWorkspace = new SuperWorkspace(
                    this,
                    'wp' + workspaceIndexToAdd,
                    newCategory,
                    this.appsByCategory[workspaceIndexToAdd],
                    Main.layoutManager.primaryMonitor,
                    false
                );
                if (this.categoryKeyOrderedList.indexOf('wp' + workspaceIndexToAdd) === -1) {
                    this.categoryKeyOrderedList.push('wp' + workspaceIndexToAdd);
                }
                this.superWorkspaces.push(superWorkspace);
                if (workspaceIndexToAdd === activeWorkspaceIndex) {

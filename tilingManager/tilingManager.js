...
var TilingManager = class TilingManager {
    constructor() {
        ...
        this.refreshAvailableLayouts();
        this.tilingIcon = new St.Icon({icon_name: 'view-dual-symbolic', style_class: 'system-status-icon'})
        Me.tilingButton.set_child(this.tilingIcon);
        Me.tilingButton.name = "tilingAction"
        Main.panel._rightBox.insert_child_at_index(Me.tilingButton, 1);
    }
    ...
    onDestroy() {
        Main.panel._rightBox.remove_child(Me.tilingButton);
        this.settingsSignals.forEach(signal =>
            this.layoutsSettings.disconnect(signal)
        );
    }
};
